...
const completionStorageKey = "lld-completed-lessons";
const openModulesStorageKey = "lld-open-modules";
const railCollapsedStorageKey = "lld-rail-collapsed";

function getActiveModuleId(modules, activeSlug) {
...
function getDefaultOpenModules(modules, activeSlug) {
  const activeModuleId = getActiveModuleId(modules, activeSlug);
  return activeModuleId ? [activeModuleId] : modules.map((module) => module.id);
}

export function LldShell({ activeSlug, modules = [], pageNav = [], children }) {
  ...
  const lessonSlugSet = useMemo(() => new Set(allLessonSlugs), [allLessonSlugs]);
  const [completedLessons, setCompletedLessons] = useState([]);
  const [openModules, setOpenModules] = useState(() => getDefaultOpenModules(modules, activeSlug));
  ...
  useEffect(() => {
    try {
      ...
      if (Array.isArray(savedOpenModules)) {
        ...
        setOpenModules(validOpenModules.length > 0 ? validOpenModules : getDefaultOpenModules(modules, activeSlug));
      } else {
        setOpenModules(getDefaultOpenModules(modules, activeSlug));
      }
  ...
  }, [activeSlug, allModuleIds, lessonSlugSet, modules]);
...
function CurriculumNav({
  ...
  if (collapsed) {
    return (
      <nav className="flex h-full flex-col items-center" aria-label="Low level design curriculum">
          ...
          <div className="grid gap-2">
            {modules.map((module, index) => {
              ...
              return (
                ...
                >
                  {module.number.replace("Module ", "") || index + 1}
                </button>
              );
            })}
          </div>
  ...
  return (
    <nav className="flex h-full flex-col" aria-label="Low level design curriculum">
...
function ModuleLessonList({ activeSlug, completedSet, module, onToggleComplete }) {
  if (module.lessons.length === 0) {
    return (
      <div className="mx-2 mt-1 rounded-md border border-dashed border-[var(--cor-border)] bg-[var(--cor-surface-2)] px-3 py-2 text-xs font-medium text-[var(--cor-muted)]">
        No lessons yet
      </div>
    );
  }
